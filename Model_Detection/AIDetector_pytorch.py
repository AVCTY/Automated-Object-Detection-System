...
class Detector(baseDet):
    ...
    def init_model(self):
        ...
        if getattr(self, "model", None) is None:
            num_classes = 8 #Change num of classes here 
            in_channels = [256, 512, 1024]
            backbone = YOLOPAFPN(self.mdepth, self.mwidth, in_channels=in_channels)
            head = YOLOXHead(num_classes, self.mwidth, in_channels=in_channels)
            model = YOLOX(backbone, head)

        model.apply(init_yolo)
        model.head.initialize_biases(1e-2)
        self.weights = 'weights/best_ckpt.pth.tar' #Change weights here
